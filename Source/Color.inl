...
Color3b::Color3b(uint8_t newRGB)
    : _x(newRGBA), _y(newRGBA), _z(newRGBA)
{
    // Nothing
}
...
Color3b::Color3b(float newR, float newG, float newB)
    : _x(castTo<uint8_t>(newR * 255.0f)), _y(castTo<uint8_t>(newG * 255.0f)), _z(castTo<uint8_t>(newB * 255.0f))
{
    // Nothing
}

/// \brief Takes apart a 32 bit integer into 8 bit increments. R is the lowest bits and A is the highest bits.
Color3b::Color3b(uint32_t newRGB)
    : _x(static_cast<uint8_t>(newRGB)), _y(static_cast<uint8_t>(newRGB << 8)), _z(static_cast<uint8_t>(newRGB << 16))
{
    // Nothing
}
...
Color3b::Color3b(const Color3b& newRGB)
    : _x(newRGB.x()), _y(newRGB.y()), _z(newRGB.z())
{
    // Nothing
}

Color3b::Color3b(const Color3f& newRGB)
    : _x(castTo<uint8_t>(newRGB.r() * 255.0f)), _y(castTo<uint8_t>(newRGB.g() * 255.0f)), _z(castTo<uint8_t>(newRGB.b() * 255.0f))
{
    // Nothing
}
...
Color4b::Color4b(float newR, float newG, float newB, float newA)
    : _x(castTo<uint8_t>(newR * 255.0f)), _y(castTo<uint8_t>(newG * 255.0f)), _z(castTo<uint8_t>(newB * 255.0f)), _w(castTo<uint8_t>(newA * 255.0f))
{
    // Nothing
}
...
Color4b::Color4B(const Color4f& newRGBA)
    : _x(castTo<uint8_t>(newRGBA.r() * 255.0f)), _y(castTo<uint8_t>(newRGBA.g() * 255.0f)), _z(castTo<uint8_t>(newRGBA.b() * 255.0f)), _w(castTo<uint8_t>(newRGBA.a() * 255.0f))
{
    // Nothing
}
