...
/// \date 2017/6/13
/// \brief Basic math helper classes and functions that don't fall into a broad
/// category.
/// \details NA
///
...
template <class T> inline T step(const T &stepSize, const T &input) {
  const T absoluteStepSize = std::abs(stepSize);
  return static_cast<T>(std::ceil(input * 1.0 / absoluteStepSize) *
                        absoluteStepSize);
}

template <class To, class From> inline To castTo(const From &from) {
  static_assert(not std::is_same<From, To>::value,
                "No need to convert the same class.");
  static_assert(std::is_arithmetic<From>::value,
                "From is an unsupported class.");
  ...
  constexpr bool validToFloat =
      fromIsDouble || fromIsInt64 || fromIsUint32 || fromIsUint64;
  static_assert(not toIsFloat || (toIsFloat && validToFloat),
                "No need to convert to float.");

  static_assert(not toIsDouble || (toIsDouble && fromIsUint64),
                "No need to convert to double.");

  constexpr bool invalidToInt16 = fromIsInt8 || fromIsInt16 || fromIsUint8;
  static_assert(not toIsInt16 || (toIsInt16 && not invalidToInt16),
                "No need to convert to int16.");

  constexpr bool invalidToInt32 =
      fromIsInt8 || fromIsInt16 || fromIsInt32 || fromIsUint8 || fromIsUint16;
  static_assert(not toIsInt32 || (toIsInt32 && not invalidToInt32),
                "No need to convert to int32.");

  constexpr bool invalidToInt64 = fromIsFloat || fromIsInt8 || fromIsInt16 ||
                                  fromIsInt32 || fromIsInt64 || fromIsUint8 ||
                                  fromIsUint16 || fromIsUint32;
  static_assert(not toIsInt64 || (toIsInt64 && not invalidToInt64),
                "No need to convert to int64.");

  constexpr bool validToUint16 =
      fromIsFloatingPoint || fromIsSigned || fromIsUint32 || fromIsUint64;
  static_assert(not toIsUint16 || (toIsUint16 && validToUint16),
                "No Need to convert to uint16.");

  constexpr bool validToUint32 =
      fromIsFloatingPoint || fromIsSigned || fromIsUint64;
  static_assert(not toIsUint32 || (toIsUint32 && validToUint32),
                "No Need to convert to uint32.");

  constexpr bool validToUint64 = fromIsFloatingPoint || fromIsSigned;
  static_assert(not toIsUint64 || (toIsUint64 && validToUint64),
                "No Need to convert to uint64.");
