...
Vector3b::Vector3b(uint8_t newXYZ) : _x(newXYZ), _y(newXYZ), _z(newXYZ) {
  // Nothing
}

Vector3b::Vector3b(uint8_t newX, uint8_t newY, uint8_t newZ)
    : _x(newX), _y(newY), _z(newZ) {
  // Nothing
}

Vector3b::Vector3b(uint32_t newXYZ)
    : _x(static_cast<uint8_t>(newXYZ)), _y(static_cast<uint8_t>(newXYZ << 8)),
      _z(static_cast<uint8_t>(newXYZ << 16)) {
  // Nothing
}

Vector3b::Vector3b(const uint8_t *const newXYZ)
    : _x(newXYZ[0]), _y(newXYZ[1]), _z(newXYZ[2]) {
  // Nothing
}

Vector3b::Vector3b(const Vector3b &newXYZ)
    : _x(newXYZ.x()), _y(newXYZ.y()), _z(newXYZ.z()) {
  // Nothing
}

uint8_t &Vector3b::x() { return _x; }

uint8_t Vector3b::x() const { return _x; }

uint8_t &Vector3b::y() { return _y; }

uint8_t Vector3b::y() const { return _y; }

uint8_t &Vector3b::z() { return _z; }

uint8_t Vector3b::z() const { return _z; }

uint8_t *Vector3b::data() { return reinterpret_cast<uint8_t *>(this); }

const uint8_t *Vector3b::data() const {
  return reinterpret_cast<const uint8_t *>(this);
}

Vector3b Vector3b::zero() { return Vector3b(0.0f, 0.0f, 0.0f); }
...
Vector4b::Vector4b(uint32_t newXYZW)
    : _x(static_cast<uint8_t>(newXYZW)), _y(static_cast<uint8_t>(newXYZW << 8)),
